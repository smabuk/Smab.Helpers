...
public static partial class MathsHelpers {
	extension<T>(T value) where T : INumber<T> {
		...
		public bool IsInRange(T minValue, T maxValueExclusive) => minValue <= value && value < maxValueExclusive;
		...
		public bool IsInRange((T MinValue, T MaxValueExclusive) range) => range.MinValue <= value && value < range.MaxValueExclusive;
	}
}
